...
$pick=mysql_query("select * from code_problem_pool order by rand() limit $_POST[no_pr]") or die('uh'.mysql_error());
...
mysql_query( "insert into test_pool(test_name,staff_id) values ('$test_name','$_SESSION[user_id]')" ) or die("podong go".mysql_error());
$curr_test_id=mysql_result(mysql_query("select max(test_id) from test_pool") ,0);

while($info = mysql_fetch_array($pick) )
{
echo "<br> ($i)".$info['prob_id']."       ".$info['question']."<br>";
mysql_query("insert into test_prob values('$curr_test_id','$info[prob_id]')") or die("poda".mysql_error());
$i++;
}
